...
//筛选按钮功能   注意点:表单中的按钮需要阻止默认行为
//点击按钮注册点击事件
//ajax发送请求
//响应后返回值模板引擎渲染页面
    //分页插件的使用
    // 1.导包    <script src="./libs/jquery.twbsPagination.js"></script>
    // 2.html页面结构    <ul id="pagination" class="pagination-sm"></ul>
    // 3.js初始化:
        //一旦总页数发生改变,需要销毁旧的插件,重新加载新插件  $('#pagination').twbsPagination('destroy');
        //重新加载插件
        //插件自带页码点击事件    onPageClick: function (event, page)
        //重新获取当前点击的页码的文章信息   改变参数
...
//发送ajax请求 根据传值id获取当前的文章信息
// 根据返回值渲染页面
//富文本域返回值有渲染失败效果   注意点:如果ajax网速极快,可能导致网络请求响应完成,但是内容还没来得及加载完毕
    //解决办法:延迟加载   setTimeout
    //富文本 文本添加方法:     tinymce.activeEditor.setContent(backData.data.content);
...
//草稿按钮注册点击事件
    ...
    //判断状态 用户提醒+网页跳转
//封装文件上传函数
    //需注意参数上传    FormData的参数追加方法   fd.append('参数名','参数值');   注意富文本的内容传递  tinymce.activeEditor.getContent()
